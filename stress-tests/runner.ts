...
import { loadBaseline, compareToBaseline } from './bun/baseline-comparator';
import { generateConsoleSummary } from './reporter';
import type { TestResult, TestConfig } from './types';
import { ALL_TESTS, PERFORMANCE_TESTS, STRESS_TESTS, SOAK_TESTS } from './config';
...
async function main() {
  ...
  console.log(`System: ${systemProfile.hostname} (${systemProfile.cpus} CPUs, ${systemProfile.memory_gb}GB RAM)`);

  // Load baseline
  const baseline = await loadBaseline();
  if (baseline) {
    console.log(`Baseline: ✓ Found (${baseline.generated_date}, v${baseline.sockethub_version})`);
  } else {
  ...
  if (options.ciMode || options.type === 'ci') {
    console.log('Mode: CI Smoke Test (1-2 minutes, crash detection only)\n');
    testsToRun = [
      {
        type: 'performance',
        platform: 'dummy',
    ...
    console.log(`Mode: ${options.type || 'all'} tests (${testsToRun.length} scenarios)\n`);
  }
  ...
  for (let i = 0; i < testsToRun.length; i++) {
    ...
    console.log(`[${i + 1}/${testsToRun.length}] ${test.type}: ${test.scenario}`);
  ...
  const reportFile = join(REPORTS_DIR, `${timestamp}-${options.type || 'all'}.json`);
...
async function runTest(
  ...
  const scenarioPath = join(
    ...
    'artillery',
    'scenarios',
    test.scenario.includes('/') ? test.scenario : `${test.type}/${test.scenario}`,
  ) + '.yml';
  ...
  try {
    ...
    const sockethubErrors = {
      credentials: report.aggregate?.counters?.['sockethub.error.credentials'] || 0,
      ...
      usersWithErrors: report.aggregate?.counters?.['sockethub.users_with_errors'] || 0,
    };

    const totalSockethubErrors = Object.values(sockethubErrors).reduce((a, b) => a + b, 0);

    const metrics = {
      latency_p50: report.aggregate?.summaries?.['sockethub.latency.echo']?.median || 0,
      latency_p95: report.aggregate?.summaries?.['sockethub.latency.echo']?.p95 || 0,
      latency_p99: report.aggregate?.summaries?.['sockethub.latency.echo']?.p99 || 0,
    ...
    const result: TestResult = {
      ...
      duration_sec: Math.round((report.aggregate?.lastCounterAt || 0) / 1000),
      metrics,
      status: 'pass',
    };
    ...
    if (vusersCreated > 0 && vusersFailed === vusersCreated) {
      result.status = 'fail';
      console.error(`      All connections failed (${vusersFailed}/${vusersCreated})`);
      return result;
    }

    if (errorsTotal > vusersCreated * 0.5) {
      result.status = 'fail';
      console.error(`      High Artillery error rate (${errorsTotal} errors, ${vusersCreated} users)`);
      return result;
    }
    ...
    if (totalSockethubErrors > 0) {
      ...
      if (errorPct > 5) {
        result.status = 'fail';
        console.error(`      Sockethub errors: ${totalSockethubErrors} (${errorPct.toFixed(1)}%)`);
        if (sockethubErrors.credentials > 0) console.error(`        - Credentials errors: ${sockethubErrors.credentials}`);
        if (sockethubErrors.echo > 0) console.error(`        - Echo errors: ${sockethubErrors.echo}`);
        if (sockethubErrors.xmpp > 0) console.error(`        - XMPP errors: ${sockethubErrors.xmpp}`);
        if (sockethubErrors.feed > 0) console.error(`        - Feed errors: ${sockethubErrors.feed}`);
        return result;
      }
    }
...
function calculateErrorRate(report: any, sockethubErrors: number = 0): number {
  const vusersCreated = report.aggregate?.counters?.['vusers.created'] || 0;
