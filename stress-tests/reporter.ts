...
export function generateConsoleSummary(results: TestResult[]): void {
  ...
  results.forEach((result, index) => {
    ...
    if (result.metrics.latency_p95) {
      console.log(`      Latency P95: ${result.metrics.latency_p95}ms ${baselineInfo}`);
    }
    if (result.metrics.throughput) {
      console.log(`      Throughput: ${result.metrics.throughput} msg/sec`);
    }
    if (result.metrics.error_rate !== undefined) {
      const errorIcon = result.metrics.error_rate > 1 ? '❌' : '✓';
      console.log(`      Error Rate: ${result.metrics.error_rate.toFixed(2)}% ${errorIcon}`);
    }
    if (result.metrics.sockethub_errors !== undefined && result.metrics.sockethub_errors > 0) {
      console.log(`      Sockethub Errors: ${result.metrics.sockethub_errors} ❌`);
    }
    console.log('');
  });
...
async function main() {
  ...
  if (showLatest) {
    ...
    const results: TestResult[] = JSON.parse(readFileSync(latestReport, 'utf-8'));
