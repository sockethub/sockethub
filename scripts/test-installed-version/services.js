...
export class ServiceManager {
  ...
  async start(suite) {
    ...
    const needsRedis = ["all", "redis", "process", "browser"].includes(suite);
  ...
  async startSockethub(binPath, runtime) {
    ...
    this.sockethubProcess.on("error", (error) => {
      this.logger
        .error("Sockethub process error", error)
        .catch(() => {});
    });
  ...
  async stop() {
    ...
    if (this.sockethubProcess) {
      try {
        ...
        await this.logger.error("Failed to stop Sockethub process", error);
      }
    }
    ...
    if (this.startedServices.length > 0) {
      try {
        ...
        await this.logger.error("Failed to stop Docker services", error);
      }
    }
  }
}
