...
Worker.prototype.boot = function () {
  ...
  this.queue.process(this.socket.id, (job, done) => {
    ...
    let platform = this.sockets.getRecord(job.data.msg.context);
    ...
    if ((! platform) && (job.data.msg['@type'] === 'cleanup')) {
      ...
      this.sockets.addRecord(platform);
    }
    ...
    d.on('error', (err) => {
      ...
      platform.module.cleanup(() => {
        ...
        this.sockets.removeRecord(platform.id); idebug('disposing of domain');
      });
      done(new Error(err));
    });
