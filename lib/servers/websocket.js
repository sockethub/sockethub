  ...
  init: function (config, server, dispatcher) {
    ...
    ws.on('request', function (req) {
      ...
      if ((typeof req.requestedProtocols.indexOf !== 'undefined') &&
        ...
        console.warn(' [ws] no valid protocols requested');
        return false;
      }

      var connection = req.accept(protoName, req.origin);
      console.info(' [ws] CONNECT ' + (new Date()) + ' ' + connection.remoteAddress);

      var disconnect = dispatcher.connect(connection);

      connection.on('close', function (reasonCode, description) {
        console.info(' [ws] DISCONNECT ' + (new Date()) + ' ' + connection.remoteAddress);
        disconnect();
      });
    });
  }
