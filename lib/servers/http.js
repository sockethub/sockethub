...
module.exports = {
  init: function (config) {
    ...
    // static files server
    var static_directory = new node_static.Server(config.EXAMPLES.LOCATION);
        ...
        */
        ...
        req.addListener('end', function () {
          ...
          if ((config.PUBLIC) && (config.PUBLIC.PATH)) {
            if (req.url.substr(1, config.PUBLIC.PATH.length) === config.PUBLIC.PATH) {
              console.debug(' [http] stripping '+config.PUBLIC.PATH+' from '+req.url);
          ...
          if (req.url.substr(0, config.PUBLIC.EXAMPLES_PATH.length) === config.PUBLIC.EXAMPLES_PATH) {
            //console.debug(' [http] stripping '+config.PUBLIC.EXAMPLES_PATH+' from '+req.url);
