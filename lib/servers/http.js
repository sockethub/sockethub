...
module.exports = {
  init: function (config) {
    ...
    // static files server
    var static_directory = new node_static.Server(config.EXAMPLES.DIRECTORY);
    ...
    function listeners(req, res) {
      ...
      req.addListener('end', function () {
        console.info(' [http] serving - ' + req.url);
        if ((config.PUBLIC) && (config.PUBLIC.WEBSOCKET_PATH)) {
          if (req.url.substr(0, config.PUBLIC.WEBSOCKET_PATH.length) === config.PUBLIC.WEBSOCKET_PATH) {
            ...
            req.url = req.url.replace(config.PUBLIC.WEBSOCKET_PATH + '/', '/');
          }
        }
