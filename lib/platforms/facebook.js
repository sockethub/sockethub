    ...
    var req = https.request({
      ...
      res.on('data', function (chunk) {
        ...
        try {
          ...
          session.error('got unknown (non-JSON) chunk from facebook post attempt to /' + job.actor.address + '/feed');
          object.message = 'got unknown (non-JSON) chunk from facebook post attempt to /' + job.actor.address + '/feed';
          gotError = true;
        }
        if (!gotError) {
