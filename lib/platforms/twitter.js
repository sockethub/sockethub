...
function initTwit(creds) {
  if ((!creds.object.consumer_key) ||
      (!creds.object.consumer_secret) ||
      (!creds.object.access_token) ||
      (!creds.object.access_token_secret)) {
    return "twitter credentials incomplete";
  } else {
    try {
      return new Twit({
        consumer_key: creds.object.consumer_key,
        consumer_secret: creds.object.consumer_secret,
        access_token: creds.object.access_token,
        access_token_secret: creds.object.access_token_secret
      });
    } catch (e) {
 ...
 * Class: Twitter
 *
 * Handles all actions related to interacting with the Twitter API.
 *
 * Uses the 'Twit' module.
 *
 * https://github.com/ttezel/twit
 *
 */
function Twitter () {}
Twitter.prototype.schema = {
  "set": {
    "credentials" : {
      "required": ['object'],
      "properties" : {
        "object": {
          "type": "object",
          "required": ['objectType', 'consumer_key', 'consumer_secret', 'access_token', 'access_token_secret'],
