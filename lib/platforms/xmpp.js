...
var Xmpp = function() {
  ...
  function registerListeners(client, handlers) {
    ...
    client.xmpp.on('stanza', listeners['stanza']);
    ...
    client.xmpp.on('chat', listeners['chat']);
    ...
    client.xmpp.on('buddy', listeners['buddy']);
    ...
    client.xmpp.on('subscribe', listeners['subscribe']);
    ...
    client.xmpp.on('unsubscribe', listeners['unsubscribe']);
    ...
    client.xmpp.on('close', listeners['close']);
    ...
    client.xmpp.on('error', listeners['error']);
  ...
  function connect(client) {
    ...
    handlers['error'] = function (error) {
      try {
        ...
        console.log('*** XMPP ERROR: failed rejecting promise ', e);
        throw new Error(e);
      }
    };
    ...
    handlers['close'] = function () {
      // FIXME
      console.log('XMPP: close received for ' + client.fullJid + '  NOT IMPLEMENTED');
    };

    var listeners = registerListeners(client, handlers);

    client.xmpp.on('online', listeners['online']);
  ...
  function getClient(actor) {
    ...
    session.getConfig('credentials').then(function (credentials) {
      ...
      if (typeof credentials[actor] === 'undefined') {
        ...
        var client = session.clientManager.get(fullJid);
        if (!client) {
          //
          ...
          //
          // client already exists, send it along
          promise.fulfill(client);
        }
      }
    }, promise.reject);
