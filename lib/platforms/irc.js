  ...
  session.clientManager.create(key, creds, {
    ...
    connect: function (key, credentials, cb) {
      ...
      var module_creds = {
        ...
        port: credentials.object.port || 6697,
        secure: (typeof credentials.object.secure === 'boolean') ? credentials.object.secure : true
      };

      function onRegister(object) {
        session.info('connected to ' + credentials.object.server);
      ...
      self.api.hookEvent(key, '*', function (message) {
          session.debug(JSON.stringify(message));
      });
    ...
    listeners: {
      '*': function (object) {
        if (typeof object.names === 'object') {
          ...
          // leave
          this.session.debug('received leave: ' + object.nickname + ' -> ' + object.target, object);
          this.session.send({
            verb: 'leave',
            actor: { address: object.nickname },
            target: [{ address: object.target }],
