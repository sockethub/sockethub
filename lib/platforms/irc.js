  ...
  session.clientManager.create(key, creds, {
    ...
    connect: function (key, credentials, cb) {
      ...
      var creds = {
        ...
        server: credentials.server || 'irc.freenode.net',
      ...
      function onRegister(object) {
        _this.session.info('connected to ' + creds.server);
      ...
      // connect...
      client = self.api.createClient(key, creds);
    },
    listeners: {
      '*': function (object) {
    ...
    addListener: function (client, key, name, func) {
      this.session.debug('addListener called! '+key+' '+name+' '+func);
      self.api.hookEvent(key, name, func);
    },
    removeListener: function (client, key, name) {
    ...
    disconnect: function (client, key, cb) {
      console.log('irc disconnect for ' + key);
...
IRC.prototype.send = function (job) {
  ...
  self._getClient(job).then(function (client) {
    ...
    client.conn.irc.raw(['PRIVMSG', job.target[0].address, job.object.text]);
    q.resolve();
  }, q.reject).fail(q.reject);
...
IRC.prototype.observe = function (job) {
  ...
  self._getClient(job).then(function (client) {
    ...
    if (job.object.objectType === 'attendance') {
      job.target.forEach(function (t) {
        client.conn.irc.raw(['WHO', t.address]);
      });
      q.resolve();
    } else {
