...
IRC.prototype._registerListeners = function (client, handlers) {
  var self = this;
  self.session.info('registering listeners for sessionId: ' + self.sessionId);
  var listeners = {
    message: {},
    pm: {},
    error: {},
  ...
  listeners.message[self.sessionId] = function (from, to, message) {
    self.session.debug("received message from " + from + '[to: ' + to + ']: '+message);
    self.session.send({
      verb: 'send',
      actor: { address: from },
      target: [{ address: to || '' }],
  ...
  listeners.pm[self.sessionId] = function (from, message) {
    self.session.debug('received pivate message: ' + from );
  ...
  listeners.join[self.sessionId] = function (channel, who) {
    self.session.debug('received join status: ' + channel + ' -> ' + who);
    self.session.send({
      verb: 'join',
      actor: { address: who },
  ...
  listeners.part[self.sessionId] = function (channel, who, reason, message) {
    self.session.debug('received leave status: ' + channel + ' -> ' + who);
    self.session.send({
      verb: 'leave',
      actor: { address: who },
      target: [{ address: channel }],
  ...
  listeners.names[self.sessionId] = function (channel, nicks) {
    //self.session.debug('received user list: #' + channel + ': ', nicks);
    var cleanNicks = [];
  ...
  listeners.topic[self.sessionId] = function (channel, topic, nick, message) {
    //self.session.debug('received topic change list: #' + channel + ':'+nick+': '+ topic);
    self.session.send({
      verb: 'update',
      actor: { address: nick },
      target: [{ address: channel }],
  ...
  listeners.nick[self.sessionId] = function (oldnick, newnick, channels, message) {
    self.session.debug('received nick change ' + oldnick + ' -> ' + newnick);
    self.session.send({
      verb: 'update',
  ...
  if (handlers.error) {
    ...
    listeners.error[self.sessionId] = function (error) {
      try {
        self.session.debug("*** IRC ERROR (rl): " + error);
        ...
        console.debug('*** IRC ERROR (rl catch): ', e);
      }
    };
  }
  ...
  listeners.raw[self.sessionId] = function (message) {
    //console.debug("IRC RAW MESSAGE: ", message);
  };
  client.irc.addListener('raw', listeners.raw[self.sessionId]);

  self.session.info('finished registering listeners for sessionId: ' + self.sessionId);
  return listeners;
};
      ...
      if ((!client) && (create)) {
        ...
        client = {
          ...
          actor: job.actor.address,
            ...
            session.info('ending irc connection ' + self.sessionId);
            this.irc.disconnect();
          },
          credentials: creds
        };
        ...
        client.listeners = mergeListeners(client, self._registerListeners(client, handlers));
        ...
        session.info('attempting connect to ' + creds.server);
...
IRC.prototype.join = function (job) {
  ...
  session.debug('join() called for ' + job.actor.address);
...
IRC.prototype.leave = function (job) {
  var q = Q.defer();
  session.debug('leave() called for ' + job.actor.address);
  var self = this;
...
IRC.prototype.send = function (job) {
  ...
  self._getClient(job, false).then(function (client) {
