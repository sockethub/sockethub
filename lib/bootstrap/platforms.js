  ...
  for (let moduleName of moduleList) {
    if (rx.test(moduleName)) {
      ...
      if (willLoad) {
        ...
        try {
          pjson = require('./../../node_modules/' + moduleName + '/package.json');
        } catch (e) {
          throw new Error(e);
        }
        ...
        if (typeof p.config !== 'object') {
          throw new Error('platform ' + platformName +
            ' must have a config property that is an object.');
        }
