  ...
  check: function redisCheck() {
    ...
    try {
      ...
      client.on('ready', function () {
        ...
        if (v[0] < 2) {
          promise.reject("Sockethub requires redis version 2.x or above for the brpop command. " +
             "Current redis version "+ client.server_info.redis_version);
        } else {
          promise.fulfill(client.server_info.redis_version);
        }
        client.quit();
      });
    } catch (e) {
      promise.reject('cannot connect to redis: ' + e);
      client.quit();
    }
    return promise;
  },
