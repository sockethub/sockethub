...
setTimeout(function poolStats() {
  ...
  console.debug(' [util] REDIS POOL: [size: '+pool.getPoolSize()+' avail:'+pool.availableObjectsCount()+' waiting:'+pool.waitingClientsCount()+']');
...
function redisGet(funcName, client, chan, cb) {
  //console.debug(' [util] --- get ID: ' + client.__name + ' to chan ' + chan);
  var responded = false;
  client[funcName](chan, function (err, replies) {
    responded = true;
    pool.release(client);
    if (err) {
      console.error(' [util] redis error ('+funcName+' '+chan+')', err);
      cb(err, null);
    } else {
      cb(err, replies);
    }
  ...
  setTimeout(function() {
    if (!responded) {
      console.error(' [util] redis.'+funcName+' never returned (5s), destroying connection. '+chan);
      pool.destroy(client);
    }
  }, 5000);
}
...
function redisHashGet(client, chan, key, cb) {
  ...
  client.hget(chan, key, function (err, replies) {
    ...
    pool.release(client);
    if (err) {
      console.error(' [util] redis error (hget '+chan+')', err);
      cb(err, null);
    } else {
      cb(err, replies);
    }
  ...
  setTimeout(function() {
    if (!responded) {
      console.error(' [util] redis.hget never returned (5s), destroying connection. '+chan);
      pool.destroy(client);
    }
  }, 5000);
}
...
function redisBlockingGet(funcName, client, chan, cb) {
  //console.debug(' [util] --- '+funcName+' ID: ' + client.__name + ' to chan ' + chan);
  var responded = false;
  ...
  //   if (!responded) {
  //     minutes = minutes + 1;
  //     console.debug(' [util] still waiting '+minutes+'m ['+funcName+':'+client.__name+' '+chan+']');
...
var redisWrapper = {
  ...
  get: function (funcName, chan, key, cb) {
    pool.acquire(function (err, client) {
      ...
      if ((funcName === 'get') || (funcName === 'hgetall')) {
        redisGet(funcName, client, chan, cb);
      } else if (funcName === 'blpop') {
        redisBlockingGet('blpop', client, chan, cb);
      } else if (funcName === 'brpop') {
        redisBlockingGet('brpop', client, chan, cb);
      } else if (funcName === 'hget') {
        redisHashGet(client, chan, key, cb);
      }
    });
  },

  set: function redisSet(funcName, chan, key, data, cb) {
    //console.log('*** SET called: ' + funcName + ' ' + chan);
    pool.acquire(function (err, client) {
      //console.log('--- ' + funcName + ' ID: '+client.__name+' to chan ' + chan+ ' KEY:'+key, data);
      if ((funcName === 'hset') && (typeof key === 'string') && (typeof data === 'string')) {
        client[funcName](chan, key, data, function (err, reply) {
          pool.release(client);
          if (err) {
              console.error(" [util] hset error: " + err);
          }
