...
var Listener = function () {
  ...
  pub.init = function (p) {
    ...
    if (p.encKey) {
      ...
      // by the dispatcher.
      session = Session(sockethubId, p.encKey);
      encKey = p.ek;
    } else {
      session = Session(sockethubId);
    }
  ...
  _.subSystem = function () {
    ...
    util.redis.get('brpop', channel.subsystem, function (err, replies){
      ...
      if (data.verb === 'cleanup') {
        ...
        if (typeof data.object.encKey !== 'undefined') {
          //console.debug(did + "received encKey: ["+data.object.encKey+"] loading session");
          encKey = data.object.encKey;
          Session = require('./session')(sockethubId, encKey);
          util.redis.set('lpush', data.actor.channel, JSON.stringify(responseObj));
           _.subSystem();
        } else {
          console.warn('did not receive encKey from dispatcher');
          throw new util.SockethubError('did not receive encKey from dispatcher, unable to initalize Session');
        }
      }
    });
  };
