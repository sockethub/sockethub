...
var Listener = function () {
  ...
  pub.init = function (p) {
    //plat, shId, ek) {
    SockethubError = (p.SockethubError) ? p.SockethubError : Error;
  ...
  _.getJob = function (processFunc) {
    ...
    client.blpop(channel.incoming, 0, function(err, replies) {
      ...
      try {
        ...
        console.error('job from queue unrecognizable (replies[1] undefined)');
        console.debug(did + 'fresh from the queue: ',replies);
        throw (new SockethubError("empty data fetched from redis queue", true));
      }
      client.quit();
      var job;
      try {
  ...
  _.subSystem = function () {
    ...
    client.blpop(channel.subsystem, 0, function(err, replies) {
      ...
      try {
        ...
        console.error(did + 'subsystem command sent bad json: ', e);
        throw new SockethubError('invalid JSON data received from redis', true);
      }
