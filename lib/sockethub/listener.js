      ...
      try {
        sessionManager.get(sid).then(function (session) {
          ...
          for (var i = 0, len = keys.length; i < len; i = i + 1) {
            ...
            var client = psession.clientManager.get(keys[i]);
            if (client) {
              //
              ...
              if (typeof client.listeners === 'object') {
                ...
                for (var type in client.listeners) {
                  for (var listener in client.listeners[type]) {
                    console.debug(' [platform:xmpp] removing '+type+' event listeners for session [' + sid + ']: ' + typeof client.listeners[type][listener]);
                    if (typeof client.listeners[type][listener] !== 'function') {
                      console.error('Why isnt ['+listener+'] a function man ['+typeof client.listeners[type][listener]+'] ? ', client.listeners[type][listener]);
                    } else {
                      client.xmpp.removeListener(type, client.listeners[type][listener]);
                    }
