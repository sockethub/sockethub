...
function Listener(p) {
  ...
  if (p.encKey) {
    ...
    // by the dispatcher.
    sessionManager = SessionManager({platform: this.platform.name,
...
Listener.prototype.shutdown = function (sids) {
  ...
  for (i = 0, len = sids.length; i < len; i = i + 1) {
    this._cleanupPlatform(sids[i]).then(function () {
      sidsCompleted = sidsCompleted + 1;
    }, function (err) {
      sidsCompleted = sidsCompleted + 1;
      console.error(this.did + 'error cleaning up sid: ' + sids[key] +
                    ' err:' + err);
    });
  }
...
Listener.prototype._getJob = function () {
  ...
  var self = this;
  util.redis.get('blpop', this.channel.incoming, function (err, replies){
    var job_json = {};
    var job;
    var bad_job = false;
...
Listener.prototype._processJob = function (job) {
  ...
  sessionManager.get(job.sessionId).then(function (session) {
    ...
    if(! handler) {
      ...
      console.info(self.did + 'sending job to platform handler');
      try {
        handler(job).then(function (obj) {
          ...
          console.error(self.did + 'problem from platform ' + e.toString());
