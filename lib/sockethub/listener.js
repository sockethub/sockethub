...
var Listener = function () {
  ...
  pub.init = function (p) {
    ...
    if (p.encKey) {
      ...
      // by the dispatcher.
      session = Session({platform: platform.name,
    ...
    session.events.on('cleanup', function (sid) {
      if (!sid) {
        pub.shutdown();
      } else {
        try {
          _.cleanupPlatform(sid).then(function () {
    ...
    setTimeout(function waitForEncKey() {
      if (session.encKeySet()) {
        console.info(did + ' encKey set, initialization complete');
        _.getJob();
      } else {
        console.debug(did + ' encKey not yet set, delaying');
        setTimeout(waitForEncKey, 1000);
      }
    }, 1000);
  };

  pub.encKeySet = function () { return session.encKeySet(); };
