...
try {
  ...
  throw new util.SockethubError("error in lib/schemas/verbs.js : "+e);
}
try {
  platforms = normalizePlatforms(platforms);
} catch (e) {
  throw new util.SockethubError("error in lib/schemas/platforms.js : "+e);
}

var lverbs, lplatforms;
try {
  lverbs = require('./../schemas/verbs.local.js');
  console.debug('loaded lib/schemas/verbs.local.js');
  lverbs = normalizeVerbs(lverbs);

} catch (e) {
  console.debug("skipping lib/schemas/verbs.local.js");
}
try {
  lplatforms = require('./../schemas/platforms.local.js');
  console.debug('loaded lib/schemas/platforms.local.js');
  lplatforms = normalizePlatforms(lplatforms);
} catch (e) {
  console.debug("skipping lib/schemas/platforms.local.js");
}
...
for (var k in lplatforms) {
  if (platforms[k]) {
    throw new util.SockethubError('entry in platforms.local.js invalid: '+k+' already defined');
  } else {
...
for (var k in lverbs) {
  if (verbs[k]) {
    throw new util.SockethubError('entry in verbs.local.js invalid: '+k+' already defined');
  } else {
    verbs[k] = lverbs[k];
  }
}
...
try {
  ...
  throw new util.SockethubError('unable to load lib/schemas/base.js ' + e, true);
}
...
if (report.errors.length !== 0) {  // protocol.js json errors
  throw new util.SockethubError('invalid format in lib/sockethub/protocol.js (which includes lib/sockethub/platform.js & verbs.js): '+
        JSON.stringify(report.errors), true);
} else {
  console.debug(' [control] lib/sockethub/protocol.js schema validated');
}
