...
var functions = (function() {
      ...
      for (var i = 0, num = config.SECRETS.length; i < num; i = i + 1) {
        if (config.SECRETS[i] === job.object.secret) {
          ...
          if (job.object.remoteStorage) {
            session.setConfig('remoteStorage', 'default', job.object.remoteStorage);
          }
  ...
  pub.set = function (job, session, resp) {
    ...
    var platform;
    var platform_name;
    if (job.platform === 'dispatcher') {
      platform_name = job.target[0].platform;
    } else {
      platform_name = job.platform;
    }

    try {
      platform = require('./../platforms/' + platform_name)();
    } catch (e) {
      resp('unable to load platform ' + platform_name);
      return;
    }
    ...
    if ((job.object.objectType === 'credentials') &&
      ...
      if (tv4.validate(job.object, platform.schema.set.credentials.properties)) {  // protocol.js json errors
        console.debug(' [dispatcher:set] job.object schema validated ');//, report);
      } else {
        resp('invalid object format ' + tv4.error.message);
        return;
      }
    } else {
    ...
    session.getPlatformSession(platform_name).then(function (psession) {
      console.info(' [dispatcher:set] setting config for platform [' +
                   platform_name + '] key ['+job.actor.address+']');
      return psession.setConfig(job.object.objectType, job.actor.address, job)
    }).then(function () {
      resp(null);
    }, function (err) {
      resp(err);
...
if (tv4.validate(proto, baseSchema)) {  
  console.debug(' [protocol] lib/sockethub/protocol.js schema validated');
} else {
  // protocol.js json errors
  throw new Error('invalid format in lib/sockethub/protocol.js (which includes lib/sockethub/platform.js & verbs.js): '+tv4.error.message);
}
...
for (var p in proto.platforms) {
  for (var v in proto.platforms[p].verbs) {
    ...
    } else if ((v === 'set') && (p !== 'dispatcher')) {
      throw new Error('verb \'set\' is a reserved verb and cannot be defined in the ' + p + ' schema [platforms.js].');
    }
  }
  proto.platforms[p].verbs['set'] = {
    "name" : "set",
    "func" : functions.set
  };
}
