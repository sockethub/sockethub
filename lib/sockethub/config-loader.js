...
function ConfigLoader(param, cmdline) {
  ...
  if (typeof param === 'string') {
    ...
    //console.debug(' [config] loading config from file: '+param);
    configFile = param;
  } else if (typeof param === 'object') {
    // param is object of config values
    //console.debug(' [config] loading config from object: ',param);
    config = param;
  }
  ...
  if (configFile) {
    try {
      ...
      try {
        ...
        console.error(' [config] unable to load config: ' + configFile, e2);
  ...
  if (config.PLATFORMS.length === 0) {
    console.error(' [config] no platforms defined in config, exiting');
