...
function ConfigLoader(param1, param2, cmdline) {
  ...
  config.BASE_PATH = path.normalize(path.resolve(require.main.filename, '../../') + '/');
  ...
  config.printSummary = function () {
    ...
    var location = urn + '://' + config.PUBLIC.DOMAIN + ':' + config.PUBLIC.PORT + config.PUBLIC.WEBSOCKET_PATH;
    console.log('    websocket url:  ' + location);
    var examples = (config.EXAMPLES.ENABLE) ? 'enabled' : 'disabled';
    if (examples) {
      console.log('');
      console.log('     examples dir:  ' + config.PUBLIC.EXAMPLES.DIRECTORY);
      console.log('     examples url:  ' + location + config.PUBLIC.EXAMPLES_PATH);
    }
