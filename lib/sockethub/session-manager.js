 ...
 *
 */
SessionManager.prototype._idx = arrayIndex;
...
// SessionManager.prototype._getSessionIfExists = arrayIndex.getRecordIfExists;

// SessionManager.prototype._addSession = arrayIndex.addRecord;

// SessionManager.prototype._idx.removeRecord = arrayIndex.removeRecord;
...
SessionManager.prototype.get = function (sid, create) {
  ...
  var session = SessionManager.prototype._idx.getRecordIfExists(sid);
  if (session) {
  ...
  redisPool.hgetall(redis_key, function (err, reply) {
    ...
    SessionManager.prototype._idx.addRecord(session);
    q.resolve(session);
  });
  ...
  if (sid) {
    ...
    if (session) {
      ...
      session.cleanup().then(function () {
        SessionManager.prototype._idx.removeRecord(sid);
        q.resolve();
      }, function () {
        SessionManager.prototype._idx.removeRecord(sid);
        q.resolve();
