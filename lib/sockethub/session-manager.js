...
var encrypt         = require('./utils/simple-crypt.js').encrypt;
var decrypt         = require('./utils/simple-crypt.js').decrypt;


// merge two objects together, o1 being the base object, o2 being the new
// object to add to o1. - all sub-properties overridden.
    ...
    if (typeof data.object.sids === 'object') {
      ...
      for (key in data.object.sids) {
        if (data.object.sids[key]) {
          ...
          console.debug('listeners.cleanup() called on ' + data.object.sids[key]);
...
SessionManager.prototype.getAllSessionIDs = function () {
  var sids =  arrayIndex.getIdentifiers();
...
SessionManager.prototype._destroyAllSessions = function () {
  var self = this;
  console.debug('SessionManager._destroyAllSessions() called');
  arrayIndex.forEachRecord(function (record) {
    if (record.sid) {
...
SessionManager.prototype.get = function (sid, create) {
  ...
  var session = arrayIndex.getRecordIfExists(sid);
  if (session) {
  ...
  if (!create) {
    console.debug('SessionManager.get(): session does not exists and create flag is false');
