 ...
 *   promise
 */
SessionManager.prototype._setConfig = function (platform, name, prop, __config) {
  var q = Q.defer();
  if (this.redis_key === '') {
  ...
  console.debug(this.log_id + ' [session:' + platform + ':' + this.sid +
               ...
               ' field: ' + name);
  var self = this;
  redisPool.hget(this.redis_key + platform, name, function (err, reply) {
    var obj;
    if (err) {
    ...
    redisPool.hset(self.redis_key + platform, name, obj, function (err) {
      if (err) {
        console.error('session.getConfig error ', err);
        q.reject(err);
      } else {
 ...
 *   prop     - name of first property segment to return
 ...
 *         // got credential object for 'silverbucket'
 *       });
 *
 * Returns:
 *
 ...
 */
SessionManager.prototype._getConfig = function (platform, name, prop, refresh) {
  var q = Q.defer();
  if (this.redis_key === '') {
    q.reject('internal error, redis_key not set!');
    return q.promise;
