...
var SessionManager = function (p) {
  ...
  this.encKey = p.encKey || undefined;
  ...
  this.subsystem.events.on('cleanup', function (data) {
    if (typeof data.object.sids === 'object') {
      ...
      for (var key in data.object.sids) {
        ...
        SessionManager.prototype.destroy(data.object.sids[key]);
      }
    } else {
...
console.log('CREATING PLATFORM INSTANCE');
      if (!reply) {
        // session does not exist. crete one.
        psession = new PlatformSession({session: self,
...
console.log('DONE - CREATING PLATFORM INSTANCE');
