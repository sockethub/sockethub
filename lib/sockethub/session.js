...
var SessionManager = function (p) {
  ...
  this.subsystem.events.on('cleanup', function (data) {
    if (typeof data.object.sids === 'object') {
      ...
      console.info(log_id + ' emitting cleanup event');
...
SessionManager.prototype.destroy = function (sid) {
  ...
  if (sid) {
    ...
    console.debug(' [session:' + this.platform + '] destroyed redis key: ' + channel);
...
var PlatformSessionWrapper = (function () {
  ...
  PlatformSession.prototype.warm = function (msg, dump) {
    //console.debug('platform: ',platform);
    if (dump) {
      console.warm(' [platform:' + this.platform + '] ' + msg, dump);
    } else {
      console.warm(' [platform:' + this.platform + '] ' + msg);
    }
  };
