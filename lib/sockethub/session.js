...
var Session = function (params) {
  var platform = (params.platform) ? params.platform : undefined;
  var sockethubId = (params.sockethubId) ? params.sockethubId : undefined;
  var encKey = (params.encKey) ? params.encKey : undefined;
  var events = new EventEmitter();
  var did = ' [session:'+platform+'] ';
  var pub = {
    events: events
  };
  ...
  pub.subsystem = Subsystem(platform, sockethubId);

  pub.subsystem.events.on('ping-with-callback', function (data, callback) {
    // data received from ping command
    var obj = {};
    if (data.object.encKey) {
      console.info(did + 'setting encKey');
      encKey = data.object.encKey;
    } else if ((data.object.requestEncKey) && (encKey)) {
      obj = {"encKey": encKey};
    } else if ((data.object.requestEncKey) && (!encKey)) {
      //console.log('actor:'+data.actor.platform+' target:'+data.target[0].platform);
      console.error(did + 'received ping request for encKey but do not possess it');
    } else {
      throw util.SockethubError(did + 'recevied ping with no encKey', true);
    }
    callback(obj);
  });

  pub.subsystem.events.on('ping-response', function (data) {
    // when we receive a response to a ping, check for encKey
    //console.debug(did + 'received ping-response');
    if ((data.object.encKey) && (platform !== 'dispatcher')) {
      encKey = data.object.encKey;
    }
  });

  pub.subsystem.events.on('cleanup', function (data) {
    if (typeof data.object.sids === 'object') {
      console.info(did+'cleanup command received.');
      for (var key in data.object.sids) {
        //console.debug(did+' calling destroy on sid: '+data.object.sids[key]);
        events.emit('cleanup', data.object.sids[key]);
        pub.destroy(data.object.sids[key]);
      }
    } else {
      console.info(did+'cleanup command received with no sids, shutdown');
      events.emit('cleanup');
    }
  });

  pub.encKeySet = function () {
  ...
  pub.get = function (sid) {
    ...
    console.debug(did+ ' session.get('+sid+') called.');
    ...
    if (sid in Session.prototype.instances) {
      console.info(' [session:' + platform + '] session: ' + sid + ', exists locally.');
    ...
    util.redis.get('get', redis_key, function (err, reply) {
      if (!reply) {
        ...
        console.info(' [session:' + platform + '] created sid ' + sid + '. ['+redis_key+']');
      } else {
        // session exists, create local session object, passing reply data
        session = new SessionInstance(sid); //, reply);
        console.info(' [session:' + platform + '] ???? retreived from redis, creating. sid ' + sid + '. [redis_key:'+redis_key+']');
      }
  ...
  pub.destroy = function (sid) {
    console.debug(did+ 'session (pub) destroy called.');
    if (sid) {
      return Session.prototype.destroy(sockethubId, sid);
    } else {
      var promise = promising();
      for (var key in Session.prototype.instances) {
        Session.prototype.destroy(sockethubId, key);
      }
      promise.fulfill();
      return promise;
    }
  ...
  var SessionInstance = function (sid) {
    console.debug(did+' SessionInstance called - sid:'+sid);
    var spub = {};
    var _ = {
    ...
    spub.isRedisKeySet = function() {
      return (_.redis_key) ? true : false;
    };
    ...
    function setConfig(platform, name, _config) {
      ...
      console.info(' [session:' + platform + ':'+sid+'] setting config with key: '+_.redis_key+platform+' field: '+name);
    ...
    function getConfig(platform, name, refresh) {
      ...
      if (!refresh) { // get from redis
        var client = redis.createClient();
        console.debug(' [session:' + platform + ':'+sid+'] getting config with key: '+_.redis_key+platform+' field: '+name);
        client.hget(_.redis_key+platform, name, function (err, reply) {
          if ((reply === null) ||
              (reply === undefined) ||
              (reply === 'undefined')) {
    ...
    spub.destroy = curry([sockethubId, sid], Session.prototype.destroy);
    ...
    function send(p) {
      ...
      console.info(' [session:' + platform + '] sending message on channel ' +
    ...
    _.purgeConfig = function (platform) {
      ...
      for (var i = 0, num = platforms.length; i < num; i = i + 1) {
        util.redis.single('del', _.redis_key+platforms[i]);
      }
    };

    // Reset the session.
    spub.__cleanup = function() {
      console.info(' [session:' + platform + '] cleaning up');
      _.purgeConfig();
      setTimeout(function () {
        _.platforms = {};
        _.secret = {};
    ...
    spub.register = function (secret) {
      console.info(' [session:' + platform + '] registering sid:' + sid);
      if (secret) {
        _.secret = secret;
        _.registered = true;
    ...
    spub.getPlatformSession = function (platform) {
      ...
      if (platform in _.platforms) {
        ...
        var psession;
        // check redis DB for existing session entry.
        util.redis.get('get', _.redis_key+platform, function (err, reply) {
          if (!reply) {
            // session does not exist. crete one.
            psession = new platformSession(platform);
            console.debug(' [session:' + platform + '] created platform session. [redis_key:'+_.redis_key+platform+']');
          } else {
            // session exists, create local session object, passing reply data
...
Session.prototype.getAllSessionIDs = function () {
  ...
  console.debug(' [session:' + platform + '] all session IDs: ',sids);
  return sids;
};
...
Session.prototype.request = function(rsSettings, method, path, mimeType, body) {
  console.debug(' [session:' + platform + '] request: '+method+' '+path);
  var promise = promising();
 ...
 *   n/a
 */
Session.prototype.destroy = function (sockethubId, sid) {
  var promise = promising();
  if (!sockethubId) {
    promise.reject('no sockethubId specified');
  }
  if (sid) {
    // delete the session hash from the redis DB
    //console.info(' [session:' + platform + '] destroying session '+sid);
    var channel = 'sockethub:' + sockethubId + ':session:' + sid;
    util.redis.single('del', channel);
    console.debug(' [session:' + platform + '] destroyed redis instances: ' + channel);
    ...
    if (session) {
      ...
      // in memory.
      console.debug(' [session:' + platform + '] cleanup '+sid);
      session.__cleanup();
    //} else {
    //  console.warn(" [session:" + platform + "] can't find session instance "+sid);
    }
