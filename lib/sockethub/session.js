...
var Session = function (params) {
  ...
  pub.get = function (sid) {
    ...
    client.get(redis_key, function (err, reply) {
      if (!reply) {
        ...
        console.info(' [session] created sid ' + sid + '. ['+redis_key+']');
      } else {
  ...
  var SessionInstance = function (sid) {
    ...
    function send(p) {
      ...
      console.info(' [session] sending message on channel ' + send_channel + " - platform: "+p.platform+' verb: '+p.verb+' status: '+p.status);
    ...
    spub.__cleanup = function() {
      console.info(' [session] cleaning up');
...
Session.prototype.destroy = function (sockethubId, sid) {
  ...
  if (sid) {
    ...
    if (session) {
      ...
      // in memory.
      console.debug(' [session] cleanup '+sid);
      session.__cleanup();
    } else {
      console.warn(" [session] can't find session instance "+sid);
    }
