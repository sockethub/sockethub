 ...
 * Event: ping
 *
 * Will emit when this platform using this session instance receives a
 * ping. Ping responses are handled automatically by the subsystem class, and
 * only should be hooked into if you'd like to do something with the data object
 * *(you don't need to reply to a ping for the sender of the ping to get a
 * response).*
 *
 * Parameters:
 *
...
/**
 * Event: ping-with-callback
 *
 * Same as 'ping', except the second parameter is the callback. If the callback
 * is not called, there is never a ping response.
 *
 * Parameters:
 *
 * data - The data object received.
 *
 * callback - Function to callback when you're ready. Ping response will not be
 *            issued until this callback is called.
 *
 */
 ...
 * Event: ping-response
 *
 * Triggered when the sender platform receives the response to their ping.
 * There is no response to this event.
 *
 * Parameters:
 *
 * data - The data received from the ping response.
 *
 */
 ...
 * Event: cleanup
 *
 * Triggered when a platforms receives a cleanup command issued by the
 * dispatchers subsystem.
 *
 * Parameters:
 *
 * data - The data object sent, usually indicating what sessions/platforms
 *        to cleanup.
 *
 * Example data object:
 *
