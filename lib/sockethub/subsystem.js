...
var Subsystem = function (platform, sockethubId) {
  ...
  console.info(did + 'platform: ' + platform + ' subscribed to ' + channel);
  ...
  function send(command, object, targetPlatform) {
    ...
    if (targetPlatform) {
      ...
      console.info(did + 'broadcasting '+command+' to '+targetPlatform+' on '+channel);
    } else {
      console.info(did + 'broadcasting '+command+' on '+channel);
    }
    var client2 = redis.createClient();
