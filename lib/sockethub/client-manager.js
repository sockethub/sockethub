...
var ClientManagerWrapper = (function () {
  ...
  function registerListeners(client, session) {
    var platform = session.platform,
        sessionId = session.getSessionID();

    function listenerWrapper(listener, sess) {
      return function () {
        var args = Array.prototype.slice.call(arguments);
        listener.apply({session: sess}, args);
      };
    }
    ...
    for (var name in client.listeners) {
      ...
      if (typeof client.indexedListeners[name] === 'undefined') {
        ...
        client.indexedListeners[name][sessionId + name] = listenerWrapper(client.listeners[name], session);
   ...
   *
   *   o.removeListener(client, key, name) -
  ...
  ClientManager.prototype.create = function (session, key, creds, o, cb) {
    ...
    o.connect.apply({session: session}, [key, creds, function (err, c) {
      ...
      client = registerListeners(client, session);
  ...
  ClientManager.prototype.add = function (session, key, client) {
    ...
    clients[platform][key].references[sessionId] = {
      session: session
    };
      ...
      setTimeout(function () {
        ...
        if ((!clients[platform]) || (!clients[platform][key])) {
          ...
          try {
            ...
            clients[platform][key].client.disconnect();
          } catch (e) {
  ...
  ClientManager.prototype.get = function (session, key, creds) {
    ...
    if (compareObjects(creds, client.credentials)) {
      ...
      //
      clients[platform][key].references[sessionId] = session;
      console.info(' [client manager:get:' + platform + ':' + sessionId +
                   '] credentials match, returning existing client. count: ' +
                   ClientManager.prototype.referenceCount(platform, key));

      return registerListeners(client, session);
    } else {
      console.log('ClientManager.get CREDENTIALS DO NOT MATCH');
      console.debug('client object credentials: ', clients[platform][key].client.credentials);
   ...
   *   return description
   */
  ClientManager.prototype.removeListeners = function (platform, sessionId, key) {
      ...
      for (var type in clients[platform][key].client.listeners) {
        ...
        for (var listener in clients[platform][key].client.listeners[type]) {
          ...
          if (typeof clients[platform][key].client.listeners[type][listener] !== 'function') {
            ...
            if (typeof clients[platform][key].client.conn.removeListener !== 'function') {
              ...
              console.debug(' [client manager:removeListeners:' + platform + ':' + sessionId + '] removing ' + type +
                        ...
                        ']');
              clients[platform][key].client.conn.removeListener(
                      type,
                      clients[platform][key].client.listeners[type][listener]
              );
            }
...
module.exports = function (session) {
  ...
  if (!instances[platform][sessionId]) {
    instances[platform][sessionId] = new CMSession(session);
  }
