...
var ClientManagerWrapper = (function () {
  ...
  ClientManager.prototype.add = function (platform, sessionId, key, client) {
    console.debug(' [client manager:' + platform + ':' + sessionId + '] add ' + key);
    if (!clients[platform]) {
      clients[platform] = {};
    }
  ...
  ClientManager.prototype.remove = function (platform, sessionId, key) {
    if (!clients[platform]) {
      clients[platform] = {};
    } else if ((clients[platform][key]) && (ClientManager.prototype.referenceCount(platform, key) <= 1)) {
      console.debug(' [client manager:' + platform + ':' + sessionId +
                    '] removal ' + key + ' [count:' +
      ...
      setTimeout(function () {
        ...
        if ((!clients[platform]) || (!clients[platform][key])) {
        ...
        } else if (ClientManager.prototype.referenceCount(platform, key) <= 1) {
          // end client
          try {
            console.info(' [client manager]:' + platform + ':' + sessionId +
                         '] ending client ' + key);
          ...
          console.debug(' [client manager:'+platform+':'+sessionId+'] client \'' +
                      key + '\' spoken for, aborting removal. decreasing count [count: ' +
                      ClientManager.prototype.referenceCount(platform, key) + ']');
        }
      }, 20000); // delay for 20s
  ...
  ClientManager.prototype.get = function (platform, sessionId, key, creds) {
    if ((!key) || (!creds)) {
      console.error(' [client manager:' + platform + ':' + sessionId +
    ...
    if (compareObjects(creds, client.credentials)) {
      ...
      //
      console.info(' [client manager:' + platform + ':' + sessionId +
                   '] credentials match, returning existing client');
      clients[platform][key].references[sessionId] = true;
      return client;
    }

    console.warn(' [client manager:' + platform + ':' + sessionId +
  ...
  ClientManager.prototype.removeListeners = function (platform, sessionId, key, connection) {
    ...
    if ((!client[platform][key]) || (!client[platform][key].client.listeners)) {
      return;
    }
    ...
    if (typeof clients[platform][key].client.listeners === 'object') {
      ...
      for (var type in clients[platform][key].client.listeners) {
        for (var listener in clients[platform][key].client.listeners[type]) {
          console.debug(' [platform:xmpp] removing ' + type +
          ...
          if (typeof clients[platform][key].client.listeners[type][listener] !== 'function') {
            console.error('Why isnt ['+listener+'] a function man [' +
            ...
            if (typeof clients[platform][key].client[connection].removeListener !== 'function') {
              console.error(' [client manager] need a removeListener function');
            } else {
...
function CMSession(platform, sessionId) {
  ...
  o.removeListeners = function (key, connection) {
    return cm.getKeys.call(cm, platform, sessionId, key, connection);
  };
  return o;
}
