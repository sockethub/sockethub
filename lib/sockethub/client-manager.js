var ClientManagerWrapper = (function () {
  ...
  ClientManager.prototype.add = function (platform, sessionId, key, client) {
    ...
    console.log(' [ClientManager:'+platform+':'+sessionId+'] add ' + key);
  ...
  ClientManager.prototype.remove = function (platform, sessionId, key) {
    ...
    if ((!clients[platform]) || (!clients[platform][key])) { return; }

    console.log(' [ClientManager:'+platform+':'+sessionId+'] removal ' + key + ' [count:' +
                  clients[platform][key].references.length + '] ', clients[platform]);

    if ((clients[platform][key]) && (clients[platform][key].references.length <= 1)) {

      setTimeout(function () {
        //console.log('ATTEMPTING CHECK: ['+platform+']['+key+']: ', clients[platform]);
        if ((!clients[platform]) || (!clients[platform][key])) {
          console.warn('[ClientManager] skipping duplicate removals, should not arrive here');
        } else if (clients[platform][key].references.length <= 1) {
          // end client
          console.log('[ClientManager:'+platform+':'+sessionId+'] ending client ' + key);
          ...
          console.log('[ClientManager:'+platform+':'+sessionId+'] client spoken for, aborting removal. decreasing count [count: ' +
...
function CMSession(platform, sessionId) {
  ...
  o.add = function (key, client) {
    if ((!key) || (!client)) {
      throw new Error('[ClientManager:' + platform + ':' + sessionId +
...
module.exports = function (platform, sessionId) {
  console.debug('[ClientManager] constructor called with [platform:'+platform+', sessionId:'+sessionId+']');
