...
var ClientManagerWrapper = (function () {
  ...
  ClientManager.prototype.get = function (platform, sessionId, key, creds) {
    ...
    //
    console.log('Comparing creds: ',creds);
    console.log('Comparing client.creds: ',client.credentials);
   ...
   *   boolean
   */
  ClientManager.prototype.move = function (platform, sessionId, oldkey, newkey, creds) {
    if ((!oldkey) || (!newkey) || (!creds)) {
      console.error(' [client manager:move:' + platform + ':' + sessionId +
                    '] move needs three parameters oldkey, newkey and credentials');
      return undefined;
    }

    var client = ClientManager.prototype.get(platform, sessionId, oldkey, creds);

    if (!client) {
      return false;
    }

    ClientManager.prototype.set(platform, sessionId, newkey, client);
    ClientManager.prototype.remove(platform, sessionId, oldkey);
    return true;
  };
...
function CMSession(platform, sessionId) {
  ...
  o.move = function (oldkey, newkey, creds) {
    return cm.get.call(cm, platform, sessionId, oldkey, newkey, creds);
  };
