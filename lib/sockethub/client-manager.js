...
var ArrayIndex    = require('./utils/array-index.js');
...
var ClientManagerWrapper = (function () {
  ...
  ClientManager.prototype.add = function (session, key, client) {
    ...
    //console.log('ADD COMPLETED client.conn: ' + typeof client.conn);
  };
      ...
      setTimeout(function () {
        ...
        if ((!clients[platform]) || (!clients[platform][key])) {
          ...
          try {
            ...
            clients[platform][key].client.disconnect.apply({session: session}, [clients[platform][key].client, key, function () {
              try {
...
console.log('--asd: ', session);
  ...
  if (!instances[platform]) {
    instances[platform] = new ArrayIndex({
      identifier: 'sid'
    });
  }

  var cmSession = instances[platform].getRecordIfExists(sessionId);
  console.log('cmSession 1: ', cmSession);
  if (!cmSession) {
    cmSession = new CMSession(session);
  console.log('cmSession 2: ', cmSession);
    instances[platform].addRecord(cmSession);
  }
  console.log('cmSession 3: ', cmSession);
