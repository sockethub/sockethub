...
Sockethub.prototype.shutdown = function () {
  ...
  return Sockethub.prototype.shutdownDispatcher(dispatcher).then(function() {
    if (lcontrol) {
...
function check() {
  ...
  util.redis.check(function (err) {
    if (err) {
      ...
      promise.reject(' [sockethub] REDIS ERROR: '+err);
      //process.exit();
    } else {
...
Sockethub.prototype.shutdownDispatcher = function (dispatcher) {
  ...
  dispatcher.shutdown();
  util.redis.clean(sockethubId).then(function () {
    promise.fulfill();
  });
...
Sockethub.prototype.initListenerController = function (root) {
  ...
  var args = ['-i', sockethubId, '-c', config.CONFIG_FILE];
  if (config.DEBUG) {
    args.push('-d');
  }
  if (config.VERBOSE) {
  ...
  var control  = spawn(root+'lib/sockethub/listener-control.js', args);
