...
Dispatcher.init = function (myPlatforms, sockethubId) {
  ...
  var encKey = encText + '' + ((Math.floor((Math.random()*10)+1) + new Date().getTime()) * sockethubId);
  this.Session = require('./session')(sockethubId, encKey);

  try {
    proto = require("./protocol.js");
  } catch (e) {
    throw new this.util.SockethubError('unable to load lib/sockethub/protocol.js ' + e, true);
  }
  try {
    baseSchema = require("./../schemas/base.js");
  } catch (e) {
    throw new this.util.SockethubError('unable to load lib/schemas/base.js ' + e, true);
  }
  ...
  if (report.errors.length !== 0) {  // protocol.js json errors
    throw new this.util.SockethubError('invalid format in lib/sockethub/protocol.js (which includes lib/sockethub/platform.js & verbs.js): '+
          JSON.stringify(report.errors), true);
  } else {
    console.debug(' [dispatcher] lib/sockethub/protocol.js schema validated');
  }
  ...
  for (var p in proto.platforms) {
    for (var v in proto.platforms[p].verbs) {
      if (typeof proto.verbs[v] === 'undefined') {
        throw new this.util.SockethubError('invalid verb ' + v + ' defined in ' + p + ' platform schema [protocol.js]', true);
      }
    }
  }
  ...
  setTimeout(function () {
    for (i = 0, num = myPlatforms.length; i < num; i = i + 1) {
      ...
      Dispatcher.sendSubSystem(key, 'ping', {timestamp: now, encKey: encKey});
    }
  ...
  setTimeout(function () {
    ...
    if (showedWarning) {
      console.error(' [dispatcher] some platforms may not have initialized properly, please check the logs.');
...
Dispatcher.sendSubSystem = function (platform, command, obj) {
  ...
  this.util.redis.set('lpush', channel, JSON.stringify({
    verb: command,
    actor: {
...
Dispatcher.getSubSystemChannel = function () {
  ...
  this.util.redis.get('brpop', _this.dispatcherChannel, function (err, replies) {
    if (err) {
