...
Dispatcher.init = function (myPlatforms, sockethubId) {
  ...
  try {
    ...
    throw 'unable to load lib/sockethub/protocol.js ' + e;
  }
  try {
    baseSchema = require("./../schemas/base.js");
  } catch (e) {
    throw 'unable to load lib/schemas/base.js ' + e;
  }
  ...
  if (report.errors.length !== 0) {  // protocol.js json errors
    throw 'invalid format in lib/sockethub/protocol.js : '+
...
Dispatcher.sendSubSystem = function (platform, command, obj) {
  ...
  this.util.redisSet('lpush', channel, JSON.stringify({
    verb: command,
    actor: {
...
Dispatcher.getSubSystemChannel = function () {
  ...
  this.util.redisGet('brpop', _this.dispatcherChannel, function (err, replies) {
    if (err) {
...
Dispatcher.subscribeOutgoing = function (sessionId, callback) {
  ...
  var _this = this;
  function getNext() {
    _this.util.redisGet('brpop', channel, function(err, incoming) {
      if (err) {
...
Dispatcher.shutdown = function () {
  ...
  (function awaitShutdown() {
    ...
    if ((notAllResponded) && (count < 3)) {
      ...
      _this.util.redisGet('keys', 'sockethub:'+_this.sockethubId+':*', function (err, keys) {
        keys.forEach(function (key, pos) {
          console.debug(' [dispatcher] deleting key ' + key);
          _this.util.redisSingle('del', key);
        });
        ...
        for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
          ...
          if (report.errors.length === 0) {  // incoming json validated
            ...
            if (typeof proto.platforms[platform].verbs[verb].func === 'function') {
              ...
              _this.util.redisSet('lpush', channel, json_o);
            }
