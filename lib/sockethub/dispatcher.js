...
Dispatcher.init = function (myPlatforms, sockethubId, proto) {
  ...
  this.sessionManager = this.SessionManager({platform: 'dispatcher', sockethubId: sockethubId, encKey: encKey});
  var _this = this;

  // verify all verbs specified in the platforms area are accounted for in the verbs section
  this.sessionManager.subsystem.events.on('ping-response', function (data) {
  ...
  this.sessionManager.subsystem.events.on('ping', function (data, callback) {
    var now = new Date().getTime();
...
Dispatcher.disconnect = function () {
  ...
  this.sessionManager.get(this.sessionId).then(function (session) {
    ...
    connection.on("message", function (message) {
      ...
      if (Dispatcher.inShutdown) {
        ...
        for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
          ...
          } else if (typeof o.target === 'undefined') {
            o.target = [];
          }
          ...
          if (report.errors.length === 0) {  // incoming json validated
            ...
            if (typeof _this.proto.platforms[platform].verbs[verb].func === 'function') {
              ...
              _this.util.redis.set('rpush', channel, json_o);
            }
