...
Dispatcher.init = function (myPlatforms, sockethubId, proto) {
  ...
  this.sessionManager.subsystem.events.on('ping', function (data) {
    var now = new Date().getTime();
    try {
      ...
      // no problem
      console.info(' [dispatcher] recevied ping from external platform '+data.actor.platform);
    }
  });
      ...
      if (Dispatcher.inShutdown) {
        ...
        for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
          ...
          var rid = m[i].rid,
              ...
              verb = m[i].verb,
              o = m[i];
          console.log('config.PLATFORMS: ['+config.PLATFORMS.indexOf(platform)+']', config.PLATFORMS);
