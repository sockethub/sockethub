...
Dispatcher.init = function (myPlatforms, sockethubId, proto) {
  ...
  this.session = this.Session({platform: 'dispatcher', sockethubId: sockethubId, encKey: encKey});
  var _this = this;
...
Dispatcher.disconnect = function () {
  ...
  this.session.get(this.sessionId).then(function (session) {
...
Dispatcher.connect = function (connection) {
  ...
  Dispatcher.session.get(sessionId).then(function (session) {
    ...
    connection.on("close", function() {
      try {
        Dispatcher.session.subsystem.send('cleanup', { sids: [ sessionId ] });
        setTimeout(function () {
          session.destroy(sessionId).then(function () {
            //session = undefined;
          });
        }, 5000);
