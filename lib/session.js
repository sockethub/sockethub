...
function Session(cfg) {
  ...
  let idebug = Debug('sockethub:worker:' + cfg.socket.id + ':' + cfg.platform),
    ...
    get: function (key, create, cb) {
      ...
      self.store.get(key, function (err, creds) {
        ...
        if ((!client) && (create)) {
          ...
          self.debug('creating connection manager record.');
          cm.create(create, function (err, client) {
            if (err) {
              self.debug('connection manager create failed: ', err);
              return cb(err);
            }
