...
function Session(cfg) {
  ...
  var store = new Store({
    namespace: 'sockethub:' + cfg.sessionID + ':store:' + cfg.platform,
  ...
  var cm = ConnectionManager(cfg.sessionID + ':platform:' + cfg.platform, {
                        id: cfg.socket.id,
                        socket: cfg.socket,
                        send: this.send,
                        store: this.store,
    ...
    get: function (key, create, cb) {
      ...
      self.store.get(key, function (err, creds) {
        ...
        if ((!client) && (create)) {
          ...
          create.timeout = (typeof create.timeout === 'number') ? create.timeout : 5000;
