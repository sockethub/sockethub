...
Dispatcher.init = function () {
  ...
  setTimeout(function () {
    ...
    console.info(' [dispatcher] finished sending pings');
  }, 1000);
...
Dispatcher.sendSubSystem = function (platform, command, obj) {
  ...
  var channel = 'sockethub:listener:' + platform + ':subsystem';
  ...
  client.publish(channel, JSON.stringify({
    verb: command,
    actor: {
      channel: 'sockethub:dispatcher:subsystem',
...
Dispatcher.subscribeSubSystemChannel = function () {
  ...
  client1.on('message', function (channel, ss_msg) {
    console.info(' [dispatcher] received message on ' + channel + ' channel : ',ss_msg);
  ...
  console.info(' [dispatcher] subscribing to dispatcher:subsystem');
  client1.subscribe('sockethub:dispatcher:subsystem');
};

Dispatcher.subscribeOutgoing = function (sessionId, callback) {
  var channel = 'sockethub:dispatcher:outgoing:' + sessionId;
  var client;
...
Dispatcher.shutdown = function () {
  ...
  for (key in this.platforms.remote) {
    Dispatcher.sendSubSystem(key, 'ping', {timestampt: now});
  }
  ...
  (function awaitShutdown() {
    ...
    for (var key in _this.platforms.remote) {
      if (_this.platforms['remote'][key]['ping']['last_received'] < now) {
    ...
    if ((notAllResponded) && (count < 3)) {
      ...
      console.info(' [dispatcher] ready to shutdown');
      promise.fulfill();
    }
    //return promise;
  })();
        ...
        for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
          ...
          if (report.errors.length === 0) {  // incoming json validated
            ...
            if (typeof proto.platforms[platform].verbs[verb].func === 'function') {
              ...
              var channel = 'sockethub:listener:' + platform + ':incoming';
              console.info(' [dispatcher:'+sessionId+'] sending verb "' + verb +
                            '" to channel ' + channel);
              var json_o = JSON.stringify(o);
              //console.log('SENDING JOB: ', json_o);
              client.rpush(channel, json_o);
            }
