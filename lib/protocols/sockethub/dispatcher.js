    ...
    if (message.type === 'utf8') {
      ...
      for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
        if (typeof m[i]['rid'] !== 'string') {
          ...
          sendError(m[i]['rid'], undefined, 'confirm', 'no platform specified');
        } else if (typeof m[i]['verb'] !== 'string') {
          sendError(m[i]['rid'], m[i]['platform'], 'confirm', 'no verb (action) specified');
        } else if (typeof proto.platforms[m[i]['platform']] !== 'object') {
          sendError(m[i]['rid'], m[i]['platform'], 'confirm', 'unknown platform receivied: '+m[i]['platform']);
        } else if (typeof proto.platforms[m[i]['platform']].verbs[m[i]['verb']] !== 'object') {
          sendError(m[i]['rid'], m[i]['platform'], 'confirm', 'unknown verb received: '+m[i]['verb']);
        }
        ...
        var rid = m[i]['rid'];
        var verb = m[i]['verb'];
        var platform = m[i]['platform'];
        var jsv = _this.JSVlib.createEnvironment();

        console.log('PROTO PLATFORMS: ',proto.platforms);
        report = jsv.validate(m[verb], proto.platforms[platform].verbs[verb].schema);
        ...
        if (report.errors.length === 0) {  // incoming json validated
          ...
          if (typeof proto.platforms[platform].verbs[verb].func === 'function') {
            proto.platforms[platform].verbs[verb].func(o, onComplete, onError);
          } else {
