...
Dispatcher.init = function () {
  ...
  setTimeout(function() {
    ...
    for (var key in proto.platforms) {
      ...
      client2.publish(channel, '{"verb":"ping","actor":{"channel":"dispatcher:subsystem"}, "object":{"timestamp":"'+now+'"}}');
    }
...
Dispatcher.connect = function(connection) {
  ...
  var sendError = function(rid, verb, msg, data) {
    ...
    this.connection.sendUTF('{"rid": "'+rid+'", "verb": "'+verb+'", "status": false, "message": "'+msg+'", "data":"'+json_data+'"}');
  };
