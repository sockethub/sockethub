...
Dispatcher.connect = function(connection) {
  ...
  // XXX i don't understand why @nilclass added this. what it's used for.
  ...
  Dispatcher.Session.get(sessionId).then(function (s) {
    session = s;
    ...
    connection.on("message", function (message) {
      ...
      if (message.type === 'utf8') {
        ...
        for (var i = 0, numObjs = m.length; i < numObjs; i = i + 1) {
          ...
          //platforms are now registered when psessions are created in the listener
          //session.addPlatform(platform);

          report = jsv.validate(o, proto.platforms[platform].verbs[verb].schema);
