  ...
  _.cleanupPlatform = function cleanupPlatform(sessionId) {
    ...
    _.getPlatformInstance(sessionId).then(function (pi) {
      ...
      if (typeof pi.cleanup === 'function') {
        platform.cleanup().then(function cleanupRes() {
    ...
    client.on("message", function (channel, message) {
      ...
      if (data.verb === 'cleanup') {
        for (var key in data.object.sids) {
          responseObj.object.sid = data.object.sids[key];
          _.cleanupPlatform(data.object.sids[key]).then(function cpR() {
            //console.info(did + 'sending subsystem response for ' + data.verb + ':'+data.object.sids[key]);
            client2.publish(data.actor.channel, JSON.stringify(responseObj));
          }, function cpE(err) {
            console.error(did + 'error cleaning up sid: '+data.object.sids[key]+ ' err:'+err);
          });
        }
      } else {
        //console.info(did + 'sending subsystem response for ' + data.verb);
        client2.publish(data.actor.channel, JSON.stringify(responseObj));
      }
