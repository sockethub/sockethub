      ...
      if (platform) {
        ...
        if(! handler) {
          ...
          session.getPlatformSession(job.platform).then(function (psession) {
            console.log(' [listener:' + _this.platform + '] sending job to platform handler');
            handler(job, psession).then(function (err, status, obj) {
              console.log( ' [listener:' + _this.platform + '] received result from platform: err:' + err + ', ' + status + ', obj:'+obj);
              jobResp(err, status, obj);
            }, function (err) {
              console.log( ' [listener:' + _this.platform + '] error returned from platform: ' + err);
              jobResp(err, false);
            });
          });
        }
