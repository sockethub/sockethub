...
Listener.init = function (platform) {
  ...
  console.info(' [listener:' + platform + '] initializing');
  this.rc = this.redis.createClient();
  this.rc.on('error', function (err) {
    console.error(' [listener:' + platform + '] error: ' + err);
  });

  this.getJob();
  this.subSystem();
  console.info(' [listener:' + platform + '] initialization complete');
};


Listener.getJob = function (processFunc) {
  console.debug (' [listener:' + this.platform + '] getJob issued for ' +
...
Listener.loadPlatform = function (platform) {
  ...
  if(! platform_module) {
    console.error(' [listener:' + this.platform + '] unable to handle jobs, no ' +
...
Listener.processJob = function (job) {
  var _this = this;
  var session = this.Session.get(job.sessionId).then(function (session) {
    var jobResp = session.getResponseHandler(job);
    console.debug(' [listener:'+_this.platform+'] processJob() called');
    ...
    try {
      ...
      if (platform) {
        ...
        if(! handler) {
          console.warn(' [listener:' + _this.platform +
          ...
          session.getPlatformSession(job.platform).then(function (psession) {
            console.info(' [listener:' + _this.platform + '] sending job to platform handler');
            handler(job, psession).then(function (err, status, obj) {
              console.info( ' [listener:' + _this.platform + '] received result from platform: err:' + err + ', ' + status + ', obj:'+obj);
              jobResp(err, status, obj);
            }, function (err) {
              console.warn( ' [listener:' + _this.platform + '] error returned from platform: ' + err);
              jobResp(err, false);
            });
          });
        }
      ...
      console.error(' [listener:' + _this.platform + '] failed to execute ' + exc);
    }
    _this.getJob();
  });
};
...
Listener.cleanupPlatform = function (sessionId) {
  ...
  if (typeof platform.cleanup === 'function') {
    Session.get(sessionId).then(function (session) {
      ...
      platform.cleanup(psession);//, sessionId);
    });
  }
...
Listener.subSystem = function () {
  console.info (' [listener:' + this.platform +
               '] subscribing to ' + this.channel.subsystem);
  var client = this.redis.createClient();
  var _this = this;
  client.on("message", function (channel, message) {
    console.info(' [listener:' + _this.platform +
    ...
    console.info(' [' + channel + '] sending reply to ' +
