    ...
    xmpp.on('close', function () {
      ...
      if (self.clients[key]) {
        if (self.clients[key].refCount <= 0) {
          ...
          // reconnect
          console.log('XMPP: there are still ['+self.clients[key].refCount+'] references, SHOULD BE reconecting client (not implemented)');
  ...
  setTimeout(function () {
    if (self.clients[key]) {
      ...
      if (self.clients[key].refCount <= 0) {
        ...
        self.clients[key].xmpp.getRoster();
      }
    }
  }, 10000); // delay for 10s
};
...
function registerListeners(fullJid, actor, xmpp, session) {
  ...
  console.log('----- registering listeners for sessionId: '+sessionId, session);
  xmpp.on('stanza', function (stanza) {
    session.log("[session: "+sessionId+"] got XMPP stanza: " + stanza);
    if (stanza.is('iq')) {
      var entries = stanza.getChild('query').getChildren('item');
  ...
  xmpp.on('error', function (error) {
    try {
      session.log("got XMPP error: " + error);
    } catch (e) {
...
var Xmpp = function() {
  ...
  _.getClient = function getClient(actor) {
    ...
    if ((_.fullJid) && (CM.existsClient(_.fullJid))) {
      ...
      var c = CM.getClient(_.fullJid);
      registerListeners(_.fullJid, actor, c, _.session);
      promise.fulfill(c);
    } else {
      _.session.getConfig('credentials').then(function (credentials) {
        ...
        if (CM.existsClient(fullJid)) {
          ...
          var c = CM.getClient(fullJid);
          registerListeners(fullJid, actor, c, _.session);
          promise.fulfill(c);
          return;
        }
        ...
        registerListeners(fullJid, actor, xmpp, _.session);
...
console.log('update ClientManager: ', CM);
    ...
    _.getClient(job.actor.address).then(function (client) {
      ...
      client.conn.send(
        ...
        }).c('show').t(job.object.state).up().c('status').t(job.object.statusText)
