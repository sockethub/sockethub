...
//
var get = function (shId, sid) {
  var promise = promising();
  sockethubId = shId;

  if (sid in Session.prototype.instances) {
    console.info(' [session] retrieved locally, sid ' + sid + '.');
...
function Session(sid, redisdata) {
    ...
    if (!refresh) { // get from redis
      ...
      client.hget(_.redis_key+platform, name, function (err, reply) {
        if ((reply === null) ||
          ...
          // we're disabling remoteStorage for direct access of credential data for now.
          // apps should handle this on thier own, and pass credentials in via. set
          //
  ...
  _.purgeConfig = function (platform) {
    ...
    for (var i = 0, num = platforms.length; i < num; i = i + 1) {
      console.info(' [session] purging config for '+_.redis_key+platforms[i]);
  ...
  pub.register = function (secret) {
    ...
    if (secret) {
      _.secret = secret;
      _.registered = true;
      _.redis_key = 'sockethub:' + sockethubId + ':session:' + sid + ':'; // needs <platform> (or '_internal') appended
  ...
  function platformSession(platform, predisdata) {
    ...
    ppub.send = function (p) {
      p.platform = platform;
      //console.log('SEND:',p);
      if (!p.verb) { throw 'psession.send() received no verb'; }
      else if (!p.actor) { throw 'psession.send() received no actor'; }
      else if (!p.status) { throw 'psession.send() received no status'; }
      else if (!p.target) { throw 'psession.send() received no target ['+p.target+':'+typeof p.target+']'; }
      else if (!p.object) { throw 'psession.send() received no object'; }
    ...
    ppub.getSessionID = pub.getSessionID;
  ...
  pub.getPlatformSession = function (platform) {
    ...
    if (platform in _.platforms) {
      // session lives locally, return it.
      console.debug(' [session] retreived locally, platform session for ' + platform + '.');
      ...
      client.get(_.redis_key+platform, function (err, reply) {
        if (!reply) {
          ...
          console.debug(' [session] created platform session for ' + platform + '.');
        } else {
          // session exists, create local session object, passing reply data
...
Session.prototype.destroy = function (sid) {
  ...
  if (sid) {
    ...
    client.del(channel);
    console.debug(' [session] destroyed redis instance: ' + channel);
