...
Session.request = function(settings, method, path, mimeType, body) {
  ...
  if(settings.storageInfo && settings.storageInfo.href && settings.bearerToken) {
    ...
    req.on('response', function(response) {
      ...
      var body = '';
      response.on('data', function(chunk) { console.log('got chunk', chunk); body += chunk; });
    ...
    console.log('retrieving '+options.path);
    req.end();
  } else {
...
Session.get = function(sid) {
  ...
  var instance = {
    ...
    send: function(platform, verb, actor, target, object, rid) {
      var channel = 'dispatcher:outgoing:' + sid;
      var client = Session.redis.createClient();
      console.log(' [session] Send message on channel ' + channel);
      client.rpush(channel, JSON.stringify({
        platform: platform,
        verb: verb,
        actor: actor,
        target: target,
