...
var get = function (sid) {
  ...
  var session_key = 'sockethub:session:'+sid+':_internal';
  var session;

  // check redis Db for existing session entry.
  client.get(session_key, function (err, reply) {
    if (!reply) {
...
function Session(sid, redisdata) {
  ...
  session_prefix = 'sockethub:session:'+sid;

  // internal function, only exposed with a curry for prefix.
  function setConfig(suffix, name, _config) {
    _.config[suffix][name] = _config;
  }

  function getConfig(suffix, name) {
    var promise = promising();
    if (_.config[suffix] === undefined) {
      return {}; // XXX TODO: check the redis store?
    }
    if (_.config[suffix][name] !== undefined) {
      return _.config[suffix][name];
    } else {
      var client = redis.createClient();
      client.hget(session_prefix+':'+suffix, name, function(cfg) {
        return cfg;
      });
    }
    return promise;
  ...
  pub.createPlatformSession = function (platform) {
    ...
    // check redis Db for existing session entry.
    client.get(session_key+':'+platform, function (err, reply) {
      if (!reply) {
        // session does not exist. crete one.
  ...
  pub.rsGet = function (module, path) {
    var promise = promising();
    var rsConfig = pub.getConfig('remoteStorage');
    if(! rsConfig) {
...
//module.exports.request = Session.prototype.request;