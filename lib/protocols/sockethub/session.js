...
function Session(sid, redisdata) {
  ...
  function setConfig(platform, name, _config) {
    ...
    var client = redis.createClient();
    console.log(' [session:'+sid+'] setting config with key: '+_.redis_key+platform+' field: '+name);
  ...
  function getConfig(platform, name, refresh) {
    ...
    function getConfigRemoteStorage() {
      if (platform !== '_internal') {
        pub.getFile(platform, name).then(function (source, resource) {
          if (typeof resource.mimeType === 'undefined') {
            setConfig(platform, name, resource).then(function () {
              promise.fulfill(resource);
            });
          } else {
            console.log(' [session] retrieved non-file from ' + source + ':/' + platform + '/' +
                          name); // + ' -- resource:'+ JSON.stringify(resource));
    ...
    if (!refresh) { // get from redis
      var client = redis.createClient();
      console.log(' [session:'+sid+'] getting config with key: '+_.redis_key+platform+' field: '+name);
      client.hget(_.redis_key+platform, name, function (err, reply) {
        if ((reply === null) ||
            (reply === undefined) ||
            (reply === 'undefined')) {
  ...
  pub.register = function (secret) {
    console.log(' [session] registering sid:'+sid);
    if (secret) {
      _.secret = secret;
      _.registered = true;
  ...
  function platformSession(platform, predisdata) {
    ...
    ppub.send = function (p) {
      ...
      if ((!p.verb) || (!p.actor) || (p.rid) || (p.target) || (!p.object)) {
        throw "received invalid object to send to client, missing basic set of properties";
      }
      send(p);
    };
