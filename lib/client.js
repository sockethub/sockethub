(function (global, factory, undefined) {
  ...
  function SockethubClient(socket) {
    var storedCredentials = new Map();
    var storedActivityObjects = new Map();
    var storedJoins = new Map();
    var self = this;
      ...
      if (typeof channel === "string") {
        switch (channel) {
          ...
          case "activity-object":
            if ((content.actor) && (content.actor["@id"])) {
              storedActivityObjects.set(content.actor["@id"], content);
            }
            break;
          case "message":
            if (content["@type"] === "join") {
              var actor = content.actor["@id"] || content.actor;
              var target = content.target["@id"] || content.target;
              var key = actor + "-" + target;
              console.log("storing join command " + key, content);
              storedJoins.set(key, content);
            } else if (content["@type"] === "leave") {
              var actor = content.actor["@id"] || content.actor;
              var target = content.target["@id"] || content.target;
              var key = actor + "-" + target;
              storedJoins.delete(key);
            }
            break;
        }
    ...
    // use as a middle ware to receive incoming Sockethub messages and unpack them
    // using Activity Stream before passing them along to the app.
    ...
    function replayActivityObjects() {
      storedActivityObjects.forEach(function (obj, key) {
        console.log("replaying Activity Obejct: " + key, obj);
        socket._emit("activity-obejct", obj);
      });
    }

    function replayCredentials() {
      storedCredentials.forEach(function (obj, key) {
        console.log("replaying credentials: " + key, obj);
    ...
    function replayJoins() {
      storedJoins.forEach(function (obj, key) {
        console.log("replaying joins: " + key, obj);
