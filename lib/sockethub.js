...
// send message to every connected socketed assosciated with the given platform instance.
...
Sockethub.prototype.__getMiddleware = function (socket, sdebug) {
...
Sockethub.prototype.__handlerStoreCredentials = function (store, sdebug) {
  return (creds) => {
...
Sockethub.prototype.__handlerQueueJob = function (socket, sdebug) {
  return (msg) => {
...
Sockethub.prototype.__handlerActivityObject = function (sdebug) {
  return (obj) => {
...
Sockethub.prototype.__handleNewConnection = function (socket) {
  const sdebug = Debug('sockethub:core  :' + socket.id), // session-specific debug messages
        ...
        middleware   = this.__getMiddleware(socket, sdebug);
  ...
  socket.on('credentials',
    middleware.chain(validate('credentials'), this.__handlerStoreCredentials(store, sdebug))
  );

  socket.on('message',
    middleware.chain(validate('message'), this.__handlerQueueJob(socket, sdebug))
  );

  // when new activity objects are created on the client side, an event is
  // fired and we receive a copy on the server side.
  socket.on('activity-object',
    middleware.chain(validate('activity-object'), this.__handlerActivityObject(sdebug))
  );
};
