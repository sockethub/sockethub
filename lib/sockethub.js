...
const debug           = Debug('sockethub:core'),
      ...
      sockethubSecret = randToken.generate(64);
...
Sockethub.prototype.boot = function () {
  ...
  this.io.on('connection', (socket) => {
    ...
    const worker = new Worker({
      ...
      sockethubSecret: sockethubSecret,
    ...
    const store = new Store({
      namespace: 'sockethub:' + sockethubId + ':worker:' + worker.id + ':store',
    ...
    socket.on('message', middleware(validate('message'), (msg) => {
      ...
      const job = queue.create(socket.id, {
        ...
        socket: socket.id,
        msg: dsCrypt.encrypt(JSON.stringify(msg), sockethubSecret)
      }).save((err) => {
        if (err) {
  ...
  this.http.listen(init.port, init.host, () => {
    ...
    debug('active platforms: ', Array.from(init.platforms.keys()));
  });
};
...
setInterval(() => {
  reportCount++;
  if (reportCount === 4) {
    rmdebug('resource report:');
    rmdebug('-- socket connections: ' + Array.from(SR.socketConnections.keys()).join(', '));
    rmdebug('-- platform instances: ' + Array.from(SR.platformInstances.keys()).join(', '));
    reportCount = 0;
  }

  rmdebug('checking platform instances for stale sockets');
