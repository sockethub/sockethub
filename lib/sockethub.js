...
Sockethub.prototype.boot = function () {
  ...
  jobs.on('job complete', function (id, result) {
    kue.Job.get(id, function (err, job) {
      ...
      if (self.io.sockets.connected[job.data.socket]) {
        ...
        self.io.sockets.connected[job.data.socket].emit('message', job.data.msg);
      } else {
  ...
  self.io
    ...
    socket.on('credentials', middleware(validate('credentials'), function (creds) {
      ...
      store.save(stream.platform, stream.actor.id, stream, function (err, reply) {
        sdebug('credentials encrypted and saved to ' + stream.actor.id);
      });
    }));

    socket.on('message', middleware(validate('message'), function (msg) {
      //sdebug('processing message ', msg);
      var job = jobs.create(socket.id, {
        title: socket.id + '-' + msg.platform + '-' + (msg.id) ? msg.id : counter++,
        socket: socket.id,
