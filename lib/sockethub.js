...
let debug       = Debug("sockethub:core"),
...
Sockethub.prototype.boot = function () {
  ...
  self.io.on("connection", function (socket) {
    let sdebug = Debug("sockethub:core:" + socket.id);
    ...
    socket.on("credentials", middleware(validate("credentials"), function (creds) {
      ...
      store.save(creds.context, creds.actor["@id"], creds, function (err, reply) {
        if (err) {
          creds.error = "error storing credentials: " + err;
          sdebug(creds.error);
          self.io.sockets.connected[job.data.socket].emit("failure", creds);
        }
    ...
    socket.on("message", middleware(validate("message"), function (msg) {
      sdebug("processing incoming message");
      let job = queue.create(socket.id, {
