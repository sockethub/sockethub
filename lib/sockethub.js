...
Sockethub.prototype.boot = function () {
  ...
  this.io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      sdebug('disconnected.');
      sdebug('creating cleanup job for ' + socket.id);
      queue.create(socket.id, {
        title: socket.id + '-cleanup',
        socket: socket.id,
...
setInterval(() => {
  debug('resources - socket connections: ' + SR.socketConnections.getIdentifiers().join(', '));
  debug('resources - platform instances: ' + SR.platformInstances.getIdentifiers().join(', '));
}, 60000)
