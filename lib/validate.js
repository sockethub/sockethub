    ...
    return function (msg, next) {
      ...
      if (type === 'activity-object') {
        ...
        if (type === 'credentials') {
          ...
          if (! tv4.getSchema('http://sockethub.org/schemas/v0/platforms/' + stream.platform + '/credentials')) {
            onfail('no credentials schema found for ' + stream.platform + ' platform', type, stream);
            return next(false);
          } else if (! tv4.validate(stream, 'http://sockethub.org/schemas/v0/platforms/' + stream.platform + '/credentials')) {
          ...
          if (! tv4.validate(stream, 'http://sockethub.org/schemas/v0/activity-stream#')) {
            onfail(tv4.error.dataPath + ': ' + tv4.error.message, type, stream);
            return next(false);
          } else if (! tv4.getSchema('http://sockethub.org/schemas/v0/platforms/' + stream.platform + '/messages')) {
            onfail('no messages schema found for ' + stream.platform + ' platform', type, stream);
            return next(false);
          } else if (! tv4.validate(stream, 'http://sockethub.org/schemas/v0/platforms/' + stream.platform + '/messages')) {
            onfail(tv4.error.dataPath + ': ' + tv4.error.message, type, stream);
            return next(false);
          }
