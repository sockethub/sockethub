...
export default class IRC implements PersistentPlatformInterface {
    ...
    join(job: ActivityStream, done: PlatformCallback) {
        this.debug.debug(`join() called for ${job.actor.id}`);
        this.getClient(job.actor.id, false, (err, client) => {
            if (err) {
                return done(err);
            }
            if (this.channels.has(job.target.name)) {
                this.debug.debug(`channel ${job.target.name} already joined`);
                return done();
            }
            // join channel
            this.jobQueue.push(() => {
            ...
            this.debug.debug(`sending join ${job.target.name}`);
    ...
    leave(job: ActivityStream, done: PlatformCallback) {
        this.debug.debug(`leave() called for ${job.actor.name}`);
        this.getClient(job.actor.id, false, (err, client) => {
            if (err) {
                return done(err);
            }
    ...
    query(job: ActivityStream, done: PlatformCallback) {
        this.debug.debug(`query() called for ${job.actor.id}`);
        this.getClient(job.actor.id, false, (err, client) => {
            if (err) {
                return done(err);
            }
    ...
    disconnect(job: ActivityStream, done: PlatformCallback) {
        this.debug.debug(`disconnect called for ${job.actor.id}`);
        this.cleanup(done);
    }

    cleanup(done: PlatformCallback) {
        this.debug.debug("cleanup() called");
    ...
    private hasJoined(channel: string) {
        this.debug.debug(`joined ${channel}`);
    ...
    private ircConnect(
        ...
        client.once("error", (err: string) => {
            this.debug.debug(`irc client 'error' occurred. `, err);
            forceDisconnect("error connecting to server.");
        });

        client.once("close", () => {
            this.debug.debug(`irc client 'close' event fired.`);
            forceDisconnect("connection to server closed.");
        });

        client.once("timeout", () => {
            this.debug.debug("timeout occurred, force-disconnect");
    ...
    private registerListeners(server: string) {
        ...
        this.irc2as.events.on("unprocessed", (s: string) => {
            this.debug.debug(`unprocessed irc message:> ${s}`);
        });
        ...
        this.irc2as.events.on("pong", (timestamp: string) => {
            this.debug.debug(`received PONG at ${timestamp}`);
            this.completeJob();
        });

        this.irc2as.events.on("ping", (timestamp: string) => {
            this.debug.debug(`received PING at ${timestamp}`);
